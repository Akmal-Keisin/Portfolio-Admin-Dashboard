...
const breadcrumbs = ref([
    ...
    {
        title: 'Edit',
        href: projectRoute.edit(props.project.slug),
    },
]);

const thumbnailPreview = ref<string | null>(props.project.thumbnail);
...
const form = useForm({
    ...
    tech_stacks: props.project.tech_stacks.map(s => s.id),
});
...
const handleSubmit = () => {
    // We use POST with _method=PUT (automatic in Inertia post if we add it or just use post to a route that handles it)
    // Actually our route is POST /{project} and it calls UpdateProjectController
    form.post(projectRoute.update(props.project.slug).url);
};
</script>
